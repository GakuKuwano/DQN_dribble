...
from dqn_ideal_soccer_world import *
...
def predict():
    ...
    actions_list = [[0.5, 0], [0.0, 0.8], [0.0, -0.8]]  # 行動(action)の取りうる値のリスト
    num_episode = 1000
...
#            env.render()
            ...
            next_state, reward, done = world.one_step(action)
#            if reward > -1:
#                reward = 1
#            else:
#                reward = -1
            ...
            if step > max_step or done:
                print('episode_{}, score_{}, {}'.format(episode_step, score, player.agent.in_goal))
                break
